...
class SublimeTasksBase(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        self.date_format = self.view.settings()
...
class NewCommand(SublimeTasksBase):
    def runCommand(self, edit):
        for region in self.view.sel():
            ...
            has_bullet = re.match(
                r'^(\s*)[' +
                re.escape(self.open_tasks_bullet) +
                re.escape(self.done_tasks_bullet) +
            ...
            if has_bullet:
                ...
                line_contents = self.view.substr(
                    line) + '\n' + grps[0] + self.open_tasks_bullet + ' '
                self.view.replace(edit, line, line_contents)

            elif 'header' in current_scope:
                header = re.match(r'^(\s*)\S+', self.view.substr(line))
                ...
                has_space = re.match(r'^(\s+)(.*)', self.view.substr(line))

                if has_space:
                    grps = has_space.groups()
                    spaces = grps[0]
                    line_contents = spaces + \
                        self.open_tasks_bullet + ' ' + grps[1]
                    self.view.replace(edit, line, line_contents)
